...
title ContactManager Jetty Run
...
echo gwk runServer -a -Denv=%Env% %Suspend% %Compiler% --debug-shmem -Dgw.server.mode=%Mode% -Dgw.port=8280
rem set JAVA_OPTS=-Xdebug -Xrunjdwp:transport=dt_shmem,server=y,suspend=%OptSuspend%,address=javadebug.3
gwb runServer -a -Denv=%Env% %Suspend% %Compiler% --debug-shmem -Dgw.server.mode=%Mode% -Dgw.port=8280
